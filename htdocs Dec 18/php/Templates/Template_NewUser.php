...
$str = "Fyll i alla fält";

if (isset ($_GET['Fel'])){
		$Fel=$_GET['Fel'];
	if($Fel==1){
		$str="Användarnamn eller Email används redan";
	}
}

 if( isset($_POST['namn']) && isset($_POST['Efternamn']) && isset($_POST['Postnummer']) && isset($_POST['postadress']) && isset($_POST['adress']) && isset($_POST['Telefon']) && isset($_POST['username']) && isset($_POST['password']) && isset($_POST['mail'])) {
	...
	$pwd = filter_input(INPUT_POST, 'password', FILTER_SANITIZE_STRING,FILTER_FLAG_STRIP_LOW);
	...
	if($row !== null){
		header("location:NewUser.php?Fel=1");
	//Användarnamn eller Mail redan använt
	}else {
	 $status = 1;
